...
namespace psmove {
    ...
    class LiveMatchMocap : public NUClear::Reactor {
    	...
    	autocal::TimeStamp start_timestamp;
		
		//handle keyboard
		void handleInput(const sf::Window& w, double time_since_start);

    public:
        /// @brief Called by the powerplant to build and setup the LiveMatchMocap reactor.
        explicit LiveMatchMocap(std::unique_ptr<NUClear::Environment> environment);
    };
